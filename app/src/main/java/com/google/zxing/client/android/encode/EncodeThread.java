...
final class EncodeThread extends Thread {
  ...
  public void run() {
    try {
      Bitmap bitmap = com.google.zxing.client.android.encode.QRCodeEncoder.encodeAsBitmap(contents, format, pixelResolution, pixelResolution);
      Message message = Message.obtain(handler, IDs.encode_succeeded);
      message.obj = bitmap;
      message.sendToTarget();
    } catch (WriterException e) {
      Log.e(TAG, "Could not encode barcode", e);
      Message message = Message.obtain(handler, IDs.encode_failed);
      message.sendToTarget();
